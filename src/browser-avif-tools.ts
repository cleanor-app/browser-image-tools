...
import { renderStillWebpImageData, readWebpSource } from './browser-webp-tools';
import { renderPngChannelView, rgbaToPngBlob, type PngChannelView } from './browser-png-tools';
...
function flattenImageDataOntoWhite(imageData: ImageData) {
  ...
  for (let index = 0; index < imageData.data.length; index += 4) {
    ...
    output[index] = Math.round(imageData.data[index] * alpha + 255 * inverseAlpha);
    output[index + 1] = Math.round(imageData.data[index + 1] * alpha + 255 * inverseAlpha);
    output[index + 2] = Math.round(imageData.data[index + 2] * alpha + 255 * inverseAlpha);
    output[index + 3] = 255;
  }
...
async function encodeImageDataToAvif(imageData: ImageData, quality: number): Promise<Uint8Array> {
  assertAvifRuntime();
...
async function loadStillRasterImageData(file: File) {
  ...
  const { context } = drawImageToCanvas(loadedImage.element, loadedImage.width, loadedImage.height);
...
export async function convertAvifToJpg(params: { file: File; quality: number }) {
...
export async function convertAvifToWebp(params: { file: File; quality: number }) {
  const source = await readAvifSource(params.file);
...
export async function convertJpgToAvif(params: { file: File; quality: number }) {
...
export async function convertPngToAvif(params: { file: File; quality: number }) {
...
export async function convertWebpToAvif(params: { file: File; quality: number }) {
...
export async function optimizeAvif(params: { file: File; preset: AvifOptimizationPreset }) {
  const source = await readAvifSource(params.file);
...
export async function renderAvifChannelBlob(params: { file: File; channel: PngChannelView }) {
  const source = await readAvifSource(params.file);
